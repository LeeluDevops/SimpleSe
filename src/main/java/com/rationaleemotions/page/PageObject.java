...
/**
 * This class represents the entry point to all page object building. Houses all
 * the logic required for loading a json file, parsing it, initialising location
 * strategies based on the values provided in the json file and also in
 * constructing different html elements.
 */
public final class PageObject {
	 ...
	 *
	 * @param context        - A {@link WebDriver} instance which would be used to
	 *                       query the elements.
	 * @param jsonFileSource - The location of the JSON file that contains the
	 *                       elements definition.
	 */
	...
	/**
	 * This method is used, when dealing with custom html elements which doesn't
	 * have the required visualisation built in by our page object model. E.g., for
	 * this could include controls such as date pickers for which there are no built
	 * in objects provided by this library (similar to {@link Button} or
	 * {@link Link} etc.,)
	 *
	 * @param fieldName - The name of the field as defined in the JSON file.
	...
	private final <E> E getElement(String fieldName, Class<E> clazz) {
		WebElement webElement = newRawElement(getJsonWebElement(fieldName)).getWebElement(page.isLocalePresent());
	...
	private final <E> List<E> getElements(String fieldName, Class<E> clazz) {
		List<WebElement> webElements = newRawElement(getJsonWebElement(fieldName))
				.getWebElements(page.isLocalePresent());
	...
	private JsonWebElement getJsonWebElement(String fieldName) {
		...
		if (element == null) {
			throw new IllegalArgumentException(
					"Unable to locate element [" + fieldName + "] in the file [" + this.jsonFileSource + "]");
		}
		return element;
	}

	private RawElement newRawElement(JsonWebElement element) {
		DecoratedSearchContext context = new DecoratedSearchContext(this.context, element.getUntil(),
				element.getWaitForSeconds());
		if (!page.isLocalePresent()) {
